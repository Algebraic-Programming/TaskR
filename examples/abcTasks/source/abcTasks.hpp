...
void abcTasks(HiCR::backend::host::L1::ComputeManager *computeManager, const HiCR::L0::Device::computeResourceList_t &computeResources)
{
  ...
  // Now creating tasks
  for (size_t i = 0; i < ITERATIONS; i++) { auto taskId = i * 3 + 1; tasks[taskId] = new taskr::Task(taskId, taskBfc); }
  for (size_t i = 0; i < ITERATIONS; i++) { auto taskId = i * 3 + 0; tasks[taskId] = new taskr::Task(taskId, taskAfc); }
  for (size_t i = 0; i < ITERATIONS; i++) { auto taskId = i * 3 + 2; tasks[taskId] = new taskr::Task(taskId, taskCfc); }
