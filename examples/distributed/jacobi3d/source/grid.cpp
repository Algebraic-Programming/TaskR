...
bool Grid::initialize()
{
 ...
 bufferSizeZ = faceSizeZ*gDepth;
  ...
  for (int k = 0; k < lt.x; k++)
  {
    ...
    channelTagX0Tags.insert(channelTagX0Tags.begin(), {{localTokenBufferKey, consumerTokenBufferSlotX0}, {localCoordinationBufferKey, localConsumerCoordinationBufferX0}, {localProducerBufferKey, localProducerCoordinationBufferX0}});
    channelTagX1Tags.insert(channelTagX1Tags.begin(), {{localTokenBufferKey, consumerTokenBufferSlotX1}, {localCoordinationBufferKey, localConsumerCoordinationBufferX1}, {localProducerBufferKey, localProducerCoordinationBufferX1}});
    channelTagY0Tags.insert(channelTagY0Tags.begin(), {{localTokenBufferKey, consumerTokenBufferSlotY0}, {localCoordinationBufferKey, localConsumerCoordinationBufferY0}, {localProducerBufferKey, localProducerCoordinationBufferY0}});
    channelTagY1Tags.insert(channelTagY1Tags.begin(), {{localTokenBufferKey, consumerTokenBufferSlotY1}, {localCoordinationBufferKey, localConsumerCoordinationBufferY1}, {localProducerBufferKey, localProducerCoordinationBufferY1}});
    channelTagZ0Tags.insert(channelTagZ0Tags.begin(), {{localTokenBufferKey, consumerTokenBufferSlotZ0}, {localCoordinationBufferKey, localConsumerCoordinationBufferZ0}, {localProducerBufferKey, localProducerCoordinationBufferZ0}});
    channelTagZ1Tags.insert(channelTagZ1Tags.begin(), {{localTokenBufferKey, consumerTokenBufferSlotZ1}, {localCoordinationBufferKey, localConsumerCoordinationBufferZ1}, {localProducerBufferKey, localProducerCoordinationBufferZ1}});
  }
 ...
 for (ssize_t localId = 0; localId < localRankCount; localId++)
 {
  ...
  t.X0.type = LOCAL; t.X0.processId = processId;
  t.X1.type = LOCAL; t.X1.processId = processId;
  t.Y0.type = LOCAL; t.Y0.processId = processId;
  t.Y1.type = LOCAL; t.Y1.processId = processId;
  t.Z0.type = LOCAL; t.Z0.processId = processId;
  t.Z1.type = LOCAL; t.Z1.processId = processId;

  t.X0.lPos.x = t.lPos.x - 1; t.X0.lPos.y = t.lPos.y;     t.X0.lPos.z = t.lPos.z;
  t.X1.lPos.x = t.lPos.x + 1; t.X1.lPos.y = t.lPos.y;     t.X1.lPos.z = t.lPos.z;
  t.Y0.lPos.x = t.lPos.x;     t.Y0.lPos.y = t.lPos.y - 1; t.Y0.lPos.z = t.lPos.z;
  t.Y1.lPos.x = t.lPos.x;     t.Y1.lPos.y = t.lPos.y + 1; t.Y1.lPos.z = t.lPos.z;
  t.Z0.lPos.x = t.lPos.x;     t.Z0.lPos.y = t.lPos.y;     t.Z0.lPos.z = t.lPos.z - 1;
  t.Z1.lPos.x = t.lPos.x;     t.Z1.lPos.y = t.lPos.y;     t.Z1.lPos.z = t.lPos.z + 1;

  if (t.X0.lPos.x == -1)   { t.X0.type = REMOTE; t.X0.lPos.x = lt.x-1; }
  if (t.X1.lPos.x == lt.x) { t.X1.type = REMOTE; t.X1.lPos.x = 0;    }
  if (t.Y0.lPos.y == -1)   { t.Y0.type = REMOTE; t.Y0.lPos.y = lt.y-1; }
  if (t.Y1.lPos.y == lt.y) { t.Y1.type = REMOTE; t.Y1.lPos.y = 0;    }
  if (t.Z0.lPos.z == -1)   { t.Z0.type = REMOTE; t.Z0.lPos.z = lt.z-1; }
  if (t.Z1.lPos.z == lt.z) { t.Z1.type = REMOTE; t.Z1.lPos.z = 0;    }
  ...
  if (t.X0.type == REMOTE)
  {
    ...
    const HiCR::L0::GlobalMemorySlot::globalKey_t remoteCoordinationBufferKey = 3 * (t.X0.processId * lt.z * lt.y * lt.x + t.lPos.z * lt.y * lt.x + t.lPos.y * lt.x + lt.x - 1) + 1;
    ...
    t.X0SendChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Producer>(*_communicationManager, sendGlobalTokenBufferSlot, sendProducerCoordinationBuffer->getSourceLocalMemorySlot(), sendConsumerCoordinationBuffer, sizeof(double) * bufferSizeX, CHANNEL_DEPTH);
    t.X0RecvChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Consumer>(*_communicationManager, recvGlobalTokenBufferSlot, recvConsumerCoordinationBuffer->getSourceLocalMemorySlot(), recvProducerCoordinationBuffer, sizeof(double) * bufferSizeX, CHANNEL_DEPTH);
  }
  ...
  if (t.X1.type == REMOTE)
  {
    ...
    // Creating producer and consumer channels
    t.X1SendChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Producer>(*_communicationManager, sendGlobalTokenBufferSlot, sendProducerCoordinationBuffer->getSourceLocalMemorySlot(), sendConsumerCoordinationBuffer, sizeof(double) * bufferSizeX, CHANNEL_DEPTH);
    t.X1RecvChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Consumer>(*_communicationManager, recvGlobalTokenBufferSlot, recvConsumerCoordinationBuffer->getSourceLocalMemorySlot(), recvProducerCoordinationBuffer, sizeof(double) * bufferSizeX, CHANNEL_DEPTH);
  }
  ...
  if (t.Y0.type == REMOTE)
  {
    ...
    const HiCR::L0::GlobalMemorySlot::globalKey_t remoteCoordinationBufferKey = 3 * (t.Y0.processId * lt.z * lt.y * lt.x + t.lPos.z * lt.y * lt.x + (lt.y - 1) * lt.x + t.lPos.x) + 1;
    ...
    // Creating producer and consumer channels
    t.Y0SendChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Producer>(*_communicationManager, sendGlobalTokenBufferSlot, sendProducerCoordinationBuffer->getSourceLocalMemorySlot(), sendConsumerCoordinationBuffer, sizeof(double) * bufferSizeY, CHANNEL_DEPTH);
    t.Y0RecvChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Consumer>(*_communicationManager, recvGlobalTokenBufferSlot, recvConsumerCoordinationBuffer->getSourceLocalMemorySlot(), recvProducerCoordinationBuffer, sizeof(double) * bufferSizeY, CHANNEL_DEPTH);
  }
  ...
  if (t.Y1.type == REMOTE)
  {
    ...
    // Creating producer and consumer channels
    t.Y1SendChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Producer>(*_communicationManager, sendGlobalTokenBufferSlot, sendProducerCoordinationBuffer->getSourceLocalMemorySlot(), sendConsumerCoordinationBuffer, sizeof(double) * bufferSizeY, CHANNEL_DEPTH);
    t.Y1RecvChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Consumer>(*_communicationManager, recvGlobalTokenBufferSlot, recvConsumerCoordinationBuffer->getSourceLocalMemorySlot(), recvProducerCoordinationBuffer, sizeof(double) * bufferSizeY, CHANNEL_DEPTH);
  }
  ...
  if (t.Z0.type == REMOTE)
  {
    ...
    const HiCR::L0::GlobalMemorySlot::globalKey_t remoteCoordinationBufferKey = 3 * (t.Z0.processId * lt.z * lt.y * lt.x + (lt.z - 1) * lt.y * lt.x + t.lPos.y * lt.x + t.lPos.x) + 1;
    ...
    // Creating producer and consumer channels
    t.Z0SendChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Producer>(*_communicationManager, sendGlobalTokenBufferSlot, sendProducerCoordinationBuffer->getSourceLocalMemorySlot(), sendConsumerCoordinationBuffer, sizeof(double) * bufferSizeZ, CHANNEL_DEPTH);
    t.Z0RecvChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Consumer>(*_communicationManager, recvGlobalTokenBufferSlot, recvConsumerCoordinationBuffer->getSourceLocalMemorySlot(), recvProducerCoordinationBuffer, sizeof(double) * bufferSizeZ, CHANNEL_DEPTH);
  }
  ...
  if (t.Z1.type == REMOTE)
  {
   // Obtaining the globally exchanged memory slots
    ...
    // Creating producer and consumer channels
    t.Z1SendChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Producer>(*_communicationManager, sendGlobalTokenBufferSlot, sendProducerCoordinationBuffer->getSourceLocalMemorySlot(), sendConsumerCoordinationBuffer, sizeof(double) * bufferSizeZ, CHANNEL_DEPTH);
    t.Z1RecvChannel = std::make_unique<HiCR::channel::fixedSize::SPSC::Consumer>(*_communicationManager, recvGlobalTokenBufferSlot, recvConsumerCoordinationBuffer->getSourceLocalMemorySlot(), recvProducerCoordinationBuffer, sizeof(double) * bufferSizeZ, CHANNEL_DEPTH);
  }
  ...
  // Creating channel 
  if (processId == 0) residualConsumerChannel = std::make_unique<HiCR::channel::fixedSize::MPSC::locking::Consumer>(*_communicationManager, residualGlobalTokenBufferSlot, residualCoordinationBufferSlot, residualGlobalCoordinationBufferSlot, sizeof(double),  processCount);
  if (processId != 0) residualProducerChannel = std::make_unique<HiCR::channel::fixedSize::MPSC::locking::Producer>(*_communicationManager, residualGlobalTokenBufferSlot, residualCoordinationBufferSlot, residualGlobalCoordinationBufferSlot, sizeof(double),  processCount);
 }
...
void Grid::reset(const uint64_t lx, const uint64_t ly, const uint64_t lz)
{
 ...
 if (t.X0.type == BOUNDARY) for (int i = t.lStart.y-gDepth; i < t.lEnd.y+gDepth; i++) for (int j = t.lStart.z-gDepth; j < t.lEnd.z+gDepth; j++) for (int d = 0; d < gDepth; d++) U[j*fs.x*fs.y + i*fs.x + d] = 0.0;
 if (t.Y0.type == BOUNDARY) for (int i = t.lStart.x-gDepth; i < t.lEnd.x+gDepth; i++) for (int j = t.lStart.z-gDepth; j < t.lEnd.z+gDepth; j++) for (int d = 0; d < gDepth; d++) U[j*fs.x*fs.y + d*fs.x + i] = 0.0;
 if (t.Z0.type == BOUNDARY) for (int i = t.lStart.x-gDepth; i < t.lEnd.x+gDepth; i++) for (int j = t.lStart.y-gDepth; j < t.lEnd.y+gDepth; j++) for (int d = 0; d < gDepth; d++) U[d*fs.x*fs.y + j*fs.x + i] = 0.0;
...
void Grid::unpack(const uint64_t lx, const uint64_t ly, const uint64_t lz, const uint32_t it)
{
  ...
  for (int d = 0; d < gDepth; d++)
   for (int z = 0; z < ls.z; z++)
    for (int y = 0; y < ls.y; y++)
     localU[fs.x*fs.y*(subGrid.lStart.z+z) + fs.x*(subGrid.lStart.y+y) + d] = subGrid.X0UnpackBuffer[bufferIdx++];
 ...
 if(subGrid.X1.type  == REMOTE)
 {
  ...
  for (int d = 0; d < gDepth; d++)
   for (int z = 0; z < ls.z; z++)
    for (int y = 0; y < ls.y; y++)
     localU[fs.x*fs.y*(subGrid.lStart.z+z) + fs.x*(subGrid.lStart.y+y) + subGrid.lEnd.x + d] = subGrid.X1UnpackBuffer[bufferIdx++];
  ...
  for (int d = 0; d < gDepth; d++)
   for (int z = 0; z < ls.z; z++)
    for (int x = 0; x < ls.x; x++)
     localU[fs.x*fs.y*(subGrid.lStart.z+z) + fs.x*d + subGrid.lStart.x + x] = subGrid.Y0UnpackBuffer[bufferIdx++];
 ...
 if(subGrid.Y1.type  == REMOTE)
  {
   ...
   for (int d = 0; d < gDepth; d++)
    for (int z = 0; z < ls.z; z++)
     for (int x = 0; x < ls.x; x++)
      localU[fs.x*fs.y*(subGrid.lStart.z+z) + fs.x*(subGrid.lEnd.y+d) + subGrid.lStart.x + x] = subGrid.Y1UnpackBuffer[bufferIdx++];
  ...
  if(subGrid.Z0.type  == REMOTE)
 {
  ...
  for (int d = 0; d < gDepth; d++)
   for (int y = 0; y < ls.y; y++)
    for (int x = 0; x < ls.x; x++)
     localU[fs.x*fs.y*d + fs.x*(subGrid.lStart.y+y) + (subGrid.lStart.x+x)] = subGrid.Z0UnpackBuffer[bufferIdx++];
 ...
 if(subGrid.Z1.type  == REMOTE)
 {
  ...
  for (int d = 0; d < gDepth; d++)
   for (int y = 0; y < ls.y; y++)
    for (int x = 0; x < ls.x; x++)
     localU[fs.x*fs.y*(subGrid.lEnd.z+d) + fs.x*(subGrid.lStart.y+y) + (subGrid.lStart.x+x)] = subGrid.Z1UnpackBuffer[bufferIdx++];
...
void Grid::pack(const uint64_t lx, const uint64_t ly, const uint64_t lz, const uint32_t it)
{
 ...
 if(subGrid.X0.type  == REMOTE)
 {
  ...
  for (int d = 0; d < gDepth; d++)
   for (int z = 0; z < ls.z; z++)
    for (int y = 0; y < ls.y; y++)
     bufferPtr[bufferIdx++] = localUn[fs.x*fs.y*(subGrid.lStart.z+z) + fs.x*(subGrid.lStart.y+y) + (subGrid.lStart.x+d)];
 }
 ...
 if(subGrid.X1.type  == REMOTE)
 {
  ...
  for (int d = 0; d < gDepth; d++)
   for (int z = 0; z < ls.z; z++)
    for (int y = 0; y < ls.y; y++)
     bufferPtr[bufferIdx++] = localUn[fs.x*fs.y*(subGrid.lStart.z+z) + fs.x*(subGrid.lStart.y+y) + (subGrid.lEnd.x-gDepth+d)];
 }
 ...
 if(subGrid.Y0.type  == REMOTE)
 {
  ...
  for (int d = 0; d < gDepth; d++)
   for (int z = 0; z < ls.z; z++)
    for (int x = 0; x < ls.x; x++)
     bufferPtr[bufferIdx++] = localUn[fs.x*fs.y*(subGrid.lStart.z+z) + fs.x*(subGrid.lStart.y+d) + (subGrid.lStart.x+x)];
 }
 ...
 if(subGrid.Y1.type  == REMOTE)
 {
  ...
  for (int d = 0; d < gDepth; d++)
   for (int z = 0; z < ls.z; z++)
    for (int x = 0; x < ls.x; x++)
     bufferPtr[bufferIdx++] = localUn[fs.x*fs.y*(subGrid.lStart.z+z) + fs.x*(subGrid.lEnd.y-gDepth+d) + (subGrid.lStart.x+x)];
 }
 ...
 if(subGrid.Z0.type  == REMOTE)
 {
  ...
  for (int d = 0; d < gDepth; d++)
   for (int y = 0; y < ls.y; y++)
    for (int x = 0; x < ls.x; x++)
     bufferPtr[bufferIdx++] = localUn[fs.x*fs.y*(subGrid.lStart.z+d) + fs.x*(subGrid.lStart.y+y) + (subGrid.lStart.x+x) ];
 }
 ...
 if(subGrid.Z1.type  == REMOTE)
 {
  ...
  for (int d = 0; d < gDepth; d++)
   for (int y = 0; y < ls.y; y++)
    for (int x = 0; x < ls.x; x++)
     bufferPtr[bufferIdx++] = localUn[fs.x*fs.y*(subGrid.lEnd.z-gDepth+d) + fs.x*(subGrid.lStart.y+y) + (subGrid.lStart.x+x)];
 }
