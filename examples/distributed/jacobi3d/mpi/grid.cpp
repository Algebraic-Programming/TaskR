...
bool Grid::initialize()
{
  ...
  for (int i = 0; i < pt.z; i++)
    for (int j = 0; j < pt.y; j++) globalProcessMapping[i][j] = (int *)calloc(sizeof(int), pt.x);
  ...
  West.type       = REMOTE;
  West.processId  = processId;
  East.type       = REMOTE;
  East.processId  = processId;
  North.type      = REMOTE;
  North.processId = processId;
  South.type      = REMOTE;
  South.processId = processId;
  Up.type         = REMOTE;
  Up.processId    = processId;
  Down.type       = REMOTE;
  Down.processId  = processId;
  ...
  for (int i = 0; i < pt.z; i++)
    for (int j = 0; j < pt.y; j++) free(globalProcessMapping[i][j]);
...
void Grid::reset()
{
  ...
  if (West.type == BOUNDARY)
    for (int i = start.y - gDepth; i < end.y + gDepth; i++)
      for (int j = start.z - gDepth; j < end.z + gDepth; j++)
        for (int d = 0; d < gDepth; d++) U[j * fs.x * fs.y + i * fs.x + d] = 0;
  if (North.type == BOUNDARY)
    for (int i = start.x - gDepth; i < end.x + gDepth; i++)
      for (int j = start.z - gDepth; j < end.z + gDepth; j++)
        for (int d = 0; d < gDepth; d++) U[j * fs.x * fs.y + d * fs.x + i] = 0;
  if (Up.type == BOUNDARY)
    for (int i = start.x - gDepth; i < end.x + gDepth; i++)
      for (int j = start.y - gDepth; j < end.y + gDepth; j++)
        for (int d = 0; d < gDepth; d++) U[d * fs.x * fs.y + j * fs.x + i] = 0;
  if (East.type == BOUNDARY)
    for (int i = start.y - gDepth; i < end.y + gDepth; i++)
      for (int j = start.z - gDepth; j < end.z + gDepth; j++)
        for (int d = 0; d < gDepth; d++) U[j * fs.x * fs.y + i * fs.x + (ps.x + gDepth + d)] = 0;
  if (South.type == BOUNDARY)
    for (int i = start.x - gDepth; i < end.x + gDepth; i++)
      for (int j = start.z - gDepth; j < end.z + gDepth; j++)
        for (int d = 0; d < gDepth; d++) U[j * fs.x * fs.y + (ps.y + gDepth + d) * fs.x + i] = 0;
  if (Down.type == BOUNDARY)
    for (int i = start.x - gDepth; i < end.x + gDepth; i++)
      for (int j = start.y - gDepth; j < end.y + gDepth; j++)
        for (int d = 0; d < gDepth; d++) U[(ps.z + gDepth + d) * fs.x * fs.y + j * fs.x + i] = 0;

  if (West.type == BOUNDARY)
    for (int i = start.y - gDepth; i < end.y + gDepth; i++)
      for (int j = start.z - gDepth; j < end.z + gDepth; j++)
        for (int d = 0; d < gDepth; d++) Un[j * fs.x * fs.y + i * fs.x + d] = 0;
  if (North.type == BOUNDARY)
    for (int i = start.x - gDepth; i < end.x + gDepth; i++)
      for (int j = start.z - gDepth; j < end.z + gDepth; j++)
        for (int d = 0; d < gDepth; d++) Un[j * fs.x * fs.y + d * fs.x + i] = 0;
  if (Up.type == BOUNDARY)
    for (int i = start.x - gDepth; i < end.x + gDepth; i++)
      for (int j = start.y - gDepth; j < end.y + gDepth; j++)
        for (int d = 0; d < gDepth; d++) Un[d * fs.x * fs.y + j * fs.x + i] = 0;
  if (East.type == BOUNDARY)
    for (int i = start.y - gDepth; i < end.y + gDepth; i++)
      for (int j = start.z - gDepth; j < end.z + gDepth; j++)
        for (int d = 0; d < gDepth; d++) Un[j * fs.x * fs.y + i * fs.x + (ps.x + gDepth + d)] = 0;
  if (South.type == BOUNDARY)
    for (int i = start.x - gDepth; i < end.x + gDepth; i++)
      for (int j = start.z - gDepth; j < end.z + gDepth; j++)
        for (int d = 0; d < gDepth; d++) Un[j * fs.x * fs.y + (ps.y + gDepth + d) * fs.x + i] = 0;
  if (Down.type == BOUNDARY)
    for (int i = start.x - gDepth; i < end.x + gDepth; i++)
      for (int j = start.y - gDepth; j < end.y + gDepth; j++)
        for (int d = 0; d < gDepth; d++) Un[(ps.z + gDepth + d) * fs.x * fs.y + j * fs.x + i] = 0;
}
...
#pragma ivdep
    ...
    if (Down.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int y = 0; y < ps.y; y++)
          for (int x = 0; x < ps.x; x++) downSendBuffer[bufferIdx++] = tmpU[fs.x * fs.y * (end.z - gDepth + d) + fs.x * (start.y + y) + (start.x + x)];
    }
    ...
    if (Up.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int y = 0; y < ps.y; y++)
          for (int x = 0; x < ps.x; x++) upSendBuffer[bufferIdx++] = tmpU[fs.x * fs.y * (start.z + d) + fs.x * (start.y + y) + (start.x + x)];
    }
    ...
    if (East.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int z = 0; z < ps.z; z++)
          for (int y = 0; y < ps.y; y++) eastSendBuffer[bufferIdx++] = tmpU[fs.x * fs.y * (start.z + z) + fs.x * (start.y + y) + (end.x - gDepth + d)];
    }
    ...
    if (West.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int z = 0; z < ps.z; z++)
          for (int y = 0; y < ps.y; y++) westSendBuffer[bufferIdx++] = tmpU[fs.x * fs.y * (start.z + z) + fs.x * (start.y + y) + (start.x + d)];
    }
    ...
    if (North.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int z = 0; z < ps.z; z++)
          for (int x = 0; x < ps.x; x++) northSendBuffer[bufferIdx++] = tmpU[fs.x * fs.y * (start.z + z) + fs.x * (start.y + d) + (start.x + x)];
    }
    ...
    if (South.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int z = 0; z < ps.z; z++)
          for (int x = 0; x < ps.x; x++) southSendBuffer[bufferIdx++] = tmpU[fs.x * fs.y * (start.z + z) + fs.x * (end.y - gDepth + d) + (start.x + x)];
    }
    ...
    if (Down.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int y = 0; y < ps.y; y++)
          for (int x = 0; x < ps.x; x++) tmpU[fs.x * fs.y * (end.z + d) + fs.x * (start.y + y) + (start.x + x)] = downRecvBuffer[bufferIdx++];
    }
    ...
    if (Up.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int y = 0; y < ps.y; y++)
          for (int x = 0; x < ps.x; x++) tmpU[fs.x * fs.y * d + fs.x * (start.y + y) + (start.x + x)] = upRecvBuffer[bufferIdx++];
    }
    ...
    if (East.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int z = 0; z < ps.z; z++)
          for (int y = 0; y < ps.y; y++) tmpU[fs.x * fs.y * (start.z + z) + fs.x * (start.y + y) + end.x + d] = eastRecvBuffer[bufferIdx++];
    }
    ...
    if (West.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int z = 0; z < ps.z; z++)
          for (int y = 0; y < ps.y; y++) tmpU[fs.x * fs.y * (start.z + z) + fs.x * (start.y + y) + d] = westRecvBuffer[bufferIdx++];
    }
    ...
    if (North.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int z = 0; z < ps.z; z++)
          for (int x = 0; x < ps.x; x++) tmpU[fs.x * fs.y * (start.z + z) + fs.x * d + start.x + x] = northRecvBuffer[bufferIdx++];
    }
    ...
    if (South.type == REMOTE)
    {
      ...
      for (int d = 0; d < gDepth; d++)
        for (int z = 0; z < ps.z; z++)
          for (int x = 0; x < ps.x; x++) tmpU[fs.x * fs.y * (start.z + z) + fs.x * (end.y + d) + start.x + x] = southRecvBuffer[bufferIdx++];
    }
  }
