...
void job2(HiCR::backend::host::L1::ComputeManager* computeManager, taskr::Runtime& taskr)
{
  ...
  auto taskAfc = computeManager->createExecutionUnit([&taskr]() { printf("Job 2 - Task A %lu\n", taskr.getCurrentTask()->getLabel()); });
  auto taskBfc = computeManager->createExecutionUnit([&taskr]() { printf("Job 2 - Task B %lu\n", taskr.getCurrentTask()->getLabel()); });
  auto taskCfc = computeManager->createExecutionUnit([&taskr]() { printf("Job 2 - Task C %lu\n", taskr.getCurrentTask()->getLabel()); });
