...
void job2(HiCR::backend::host::L1::ComputeManager* computeManager, taskr::Runtime& taskr)
{
  ...
  {
    size_t taskId = i * 3 + 2;
    auto taskfc = computeManager->createExecutionUnit([taskId]() { printf("Job 2 - Task C %lu\n", taskId); });
  ...
  {
    size_t taskId = i * 3 + 1;
    auto taskfc = computeManager->createExecutionUnit([taskId]() { printf("Job 2 - Task B %lu\n", taskId); });
    tasks[taskId] = new HiCR::tasking::Task(taskfc);
  }
  ...
  {
    size_t taskId = i * 3 + 0;
    auto taskfc = computeManager->createExecutionUnit([taskId]() { printf("Job 2 - Task A %lu\n", taskId); });
    tasks[taskId] = new HiCR::tasking::Task(taskfc);
  }
