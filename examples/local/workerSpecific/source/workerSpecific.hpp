...
void workFc()
{
  ...
  // Sanity check
  if ((int)taskLabel != currentCPUId) { fprintf(stderr, "Task label (%lu) does not coincide with the current CPU id! (%d)\n", taskLabel, currentCPUId); std::abort(); }
}
...
void workerSpecific(taskr::Runtime& taskr, const size_t workerCount)
{
  ...
  for (size_t i = 0; i < workerCount; i++) if (i % 2 == 0) taskr.addTask(new taskr::Task(i, workTaskfc, i));
